...
from app.core.model_router import ModelProvider, ModelRouter, TaskType
...
Respond with ONLY the category word, nothing else."""
            ...
            if not response.parts or len(response.parts) == 0:
                logger.warning(
                    "AI response has no parts (possibly quota exceeded or blocked), falling back to keywords"
                )
                return self._classify_with_keywords(user_message)
    ...
    def _classify_with_keywords(self, user_message: str) -> str:
        ...
        # Search keywords
            ...
            word in message_lower
            for word in ["search", "find", "look up", "google", "what is", "who is", "latest"]
        ):
            return "SEARCH"

        # Reasoning keywords
        elif any(
            word in message_lower for word in ["analyze", "compare", "evaluate", "pros and cons"]
        ):
            return "REASONING"
    ...
    async def create_execution_plan(
        ...
        if intent["task_type"] == TaskType.WEB_SEARCH:
            plan = [
                ...
                {"step": 2, "action": "analyze_results", "tool": None, "parameters": {}},
                {"step": 3, "action": "generate_response", "tool": None, "parameters": {}},
            ]
        elif intent["task_type"] == TaskType.CODE_EXECUTION:
            plan = [
                {"step": 1, "action": "extract_code", "tool": None, "parameters": {}},
                {"step": 2, "action": "execute_code", "tool": "code_exec", "parameters": {}},
                {"step": 3, "action": "format_result", "tool": None, "parameters": {}},
            ]
            ...
            plan = [
                ...
                {"step": 2, "action": "deep_analysis", "tool": None, "parameters": {}},
                {"step": 3, "action": "synthesize_findings", "tool": None, "parameters": {}},
            ]
        else:
            # Simple query - direct response
            plan = [{"step": 1, "action": "generate_response", "tool": None, "parameters": {}}]
