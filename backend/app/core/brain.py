...
class AIBrain:
    ...
    async def generate(
        ...
        try:
            ...
            if provider == ModelProvider.GEMINI and self.gemini_client:
                ...
                return await self._perplexity_generate(
            ...
            if provider != ModelProvider.OLLAMA and self.ollama_available:
                ...
                return await self._ollama_generate(
                    messages, model or settings.ollama_model, temperature, max_tokens
                )
            raise
    ...
    async def stream_generate(
        ...
        use_ollama: bool = False,
        provider: ModelProvider | None = None,
        image_data: dict[str, Any] | None = None,  # For vision requests
        audio_data: dict[str, Any] | None = None,  # For multimodal audio
        ...
        try:
            ...
            if provider == ModelProvider.GEMINI and self.gemini_client:
                async for chunk in self._gemini_stream(
                    messages, model, temperature, max_tokens, tools, image_data, audio_data
                ):
                    yield chunk
            elif provider == ModelProvider.GROQ and self.groq_client:
                ...
                async for chunk in self._perplexity_stream(
                    messages, model, temperature, max_tokens, tools
                ):
                    yield chunk
            elif provider == ModelProvider.OLLAMA and self.ollama_available:
    ...
    async def _gemini_generate(self, messages, model, temperature, max_tokens, tools=None):
        ...
        try:
            ...
            model_instance = self.gemini_client.GenerativeModel(model_name, **model_kwargs)
    ...
    async def _gemini_stream(
        self, messages, model, temperature, max_tokens, tools=None, image_data=None, audio_data=None
    ):
        ...
        try:
            ...
            if image_data:
                ...
                logger.info(
                    f"Creating multimodal request with audio: {audio_data.get('name', 'audio.wav')}"
                )
    ...
    def _convert_to_gemini_messages(self, messages: list[dict[str, str]]) -> tuple:
        """
