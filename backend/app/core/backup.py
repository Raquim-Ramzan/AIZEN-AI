...
class BackupManager:
    ...
    async def create_full_backup(self) -> Dict[str, Any]:
        ...
        if self.sqlite_db.exists():
            try:
                ...
                if self.compress:
                    await self._backup_file_compressed(self.sqlite_db, sqlite_backup.with_suffix(".db.gz"))
                    manifest["components"]["sqlite"] = {"path": str(sqlite_backup.with_suffix(".db.gz")), "size": sqlite_backup.with_suffix(".db.gz").stat().st_size if sqlite_backup.with_suffix(".db.gz").exists() else 0}
                else:
                    shutil.copy2(self.sqlite_db, sqlite_backup)
                    manifest["components"]["sqlite"] = {"path": str(sqlite_backup), "size": sqlite_backup.stat().st_size}
        ...
        if self.chroma_dir.exists():
            try:
                ...
                if self.compress:
                    ...
                    manifest["components"]["chromadb"] = {"path": str(chroma_backup.with_suffix(".tar.gz"))}
                else:
        ...
        if self.core_memory_file.exists():
            try:
                ...
                if self.compress:
                    await self._backup_file_compressed(self.core_memory_file, memory_backup.with_suffix(".json.gz"))
                    manifest["components"]["core_memory"] = {"path": str(memory_backup.with_suffix(".json.gz"))}
                else:
    ...
    async def export_data(self, format: str = "json") -> Path:
        ...
        # Export core memory
        if self.core_memory_file.exists():
            async with aiofiles.open(self.core_memory_file, 'r') as f:
