...
from app.tools import CalendarTool, CodeExecTool, FileOpsTool, SystemTool, WebSearchTool
...
async def get_conversations(
    request: Request, limit: int = 50, user_id: str = Depends(require_api_key)
):
...
async def create_conversation(
    request: Request, conv: ConversationCreate, user_id: str = Depends(require_api_key)
):
...
async def get_conversation(
    request: Request, conversation_id: str, user_id: str = Depends(require_api_key)
):
    ...
    try:
        ...
        if not chat_req.conversation_id:
            chat_req.conversation_id = await conv_manager.create_conversation(
                user_id=user_id, title=chat_req.message[:50], metadata={}
            )

        # Add user message
        await conv_manager.add_message(chat_req.conversation_id, "user", chat_req.message)
        ...
        logger.info(
            f"RAG context: {rag_context['metadata']['memory_hits']} memories, "
...
- "how's my CPU?" -> use get_cpu_info
        ...
        if chat_req.model_provider and chat_req.model_name:
            ...
            provider, model = await task_planner.select_model_provider(
                intent, manual_provider=chat_req.preferred_provider
            )
        ...
        tools_param = (
            SYSTEM_TOOLS
            if provider in [ModelProvider.GEMINI, ModelProvider.GROQ, ModelProvider.PERPLEXITY]
            else None
        )
        ...
        try:
            ...
            if rag_result["facts_extracted"] > 0 or rag_result["indexed"]:
                logger.info(
                    f"RAG processed: {rag_result['facts_extracted']} facts, "
        ...
        return ChatResponse(
            ...
            provider=used_provider,
            model=used_model,
            metadata={"tool_calls": tool_calls, "pending_operations": pending_operations}
            if (tool_calls or pending_operations)
            else None,
        )
...
async def update_preference(
    request: Request, update: MemoryUpdate, user_id: str = Depends(require_api_key)
):
...
async def search_memory(
    request: Request, query: str, limit: int = 5, user_id: str = Depends(require_api_key)
):
    """Search memory"""
    try:
        vector_store = request.app.state.vector_store
...
async def add_core_fact(
    request: Request, fact_data: CoreFactCreate, user_id: str = Depends(require_api_key)
):
...
async def update_core_fact(
    request: Request, fact_data: CoreFactUpdate, user_id: str = Depends(require_api_key)
):
    """Update an existing core memory fact"""
    try:
        core_memory = request.app.state.core_memory
        success = await core_memory.update_core_fact(
            user_id=user_id, fact_id=fact_data.fact_id, new_fact=fact_data.new_fact
        )
...
async def clear_core_facts(
    request: Request, clear_data: CoreMemoryClear, user_id: str = Depends(require_api_key)
):
...
async def get_settings():
    ...
    return {
        "providers": {
            ...
            "ollama": {"configured": True, "models": [settings.ollama_model]},
        },
        "model_preferences": {
...
async def select_model(select_req: ModelSelectRequest):
    ...
    try:
        ...
        provider, model = router.select_model(
            task_type,
            manual_provider=ModelProvider(select_req.manual_provider)
            if select_req.manual_provider
            else None,
...
async def get_rag_stats(request: Request, user_id: str = Depends(require_api_key)):
    ...
    try:
        ...
        stats = (
            rag_manager.get_stats(user_id)
            if hasattr(rag_manager, "get_stats_for_user")
            else rag_manager.get_stats()
        )
        return stats
    except Exception as e:
...
async def summarize_conversation(request: Request, conversation_id: str, force: bool = False):
    ...
    try:
        ...
        if summary:
            return {"status": "completed", "conversation_id": conversation_id, "summary": summary}
        else:
            return {
                "status": "skipped",
                "conversation_id": conversation_id,
...
async def get_audit_logs(request: Request, event_type: str = None, limit: int = 100):
    ...
    try:
        ...
        events = await audit_logger.get_events(event_type=event_type_enum, limit=limit)
...
async def add_smart_memory(request: Request, fact: str, importance: str = "normal"):
    ...
    try:
        ...
        result = await smart_memory.add_fact_smart(fact=fact, importance=importance, source="api")
        return result
    except Exception as e:
