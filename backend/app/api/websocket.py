...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        ...
        manager.disconnect(client_id)
        try:
            await manager.send_message(client_id, {"type": "error", "error": str(e)})
        except Exception:
...
async def handle_message(client_id: str, message_data: dict[str, Any], websocket: WebSocket):
    ...
    try:
        ...
        if not user_message:
            await manager.send_message(
                client_id, {"type": "error", "error": "No message content provided"}
            )
            return
        ...
        if not conversation_id:
            ...
            conversation_id = await conv_manager.create_conversation(
                user_id=client_id, title=title, metadata={"source": "chat"}
            )
        ...
        user_msg_id = await conv_manager.add_message(
            conversation_id, "user", user_message, metadata=metadata
        )
        ...
        if attached_file:
            ...
            if file_type.startswith("image/") and file_data:
                image_data = {"name": file_name, "type": file_type, "data": file_data}
                logger.info("Image detected - will use Gemini Vision for analysis")
            ...
            # Check if it's an audio file for multimodal audio processing
                ...
                audio_data = {"name": file_name, "type": file_type, "data": file_data}
                logger.info("Audio detected - will use Gemini Multimodal Audio")
...
- "how's my CPU?" -> use get_cpu_info
        ...
        tools_to_use = None
        if (
            intent.get("requires_tools")
            or "open" in user_message.lower()
        ...
        if image_data or audio_data:
            ...
            if audio_data:
                logger.info("Forcing Gemini provider for audio analysis")
            else:
                logger.info("Forcing Gemini provider for image analysis")
        ...
        try:
            ...
            async for chunk in ai_brain.stream_generate(
                ...
                await manager.send_message(
                    client_id,
                    {"type": "token", "content": chunk, "conversation_id": conversation_id},
                )

        except Exception as e:
            logger.error(f"Streaming error: {e}")
            await manager.send_message(
                client_id, {"type": "error", "error": str(e), "conversation_id": conversation_id}
            )
            return
        ...
        if tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                result = await system_executor.execute_tool_call(
                    tool_name=tool_name, parameters=tool_params, user_id=client_id
                )
        ...
        # Send completion
        completion_metadata = {"provider": str(provider), "model": model}
