...
class RAGManager:
    ...
    async def get_context_for_query(
        ...
        if self.core_memory:
            try:
                result["core_memory_context"] = await self.core_memory.get_system_prompt_context(
                    user_id
                )
        ...
        if self.vector_store:
            try:
                rag_result = await self.vector_store.retrieve_context(
                    ...
                    max_memories=max_items * 2
                    if use_reranking
                    else max_items
                    if include_memories
                    else 0,  # Get more for reranking
                    max_conversations=max_items * 2
                    if use_reranking
                    else max_items
        ...
        if use_reranking and (memories or past_convos):
            try:
                ...
                if memories:
                    ...
                    memory_docs = [
                        {"content": m.get("content", m.get("document", "")), **m} for m in memories
                    ]
                ...
                if past_convos:
                    convo_docs = [
                        {"content": c.get("content", c.get("document", "")), **c}
    ...
    async def process_conversation_exchange(
            ...
            try:
                ...
                for fact_text in extracted_facts:
                    add_result = await smart_memory.add_fact_smart(
                        fact=fact_text, importance="normal", source="conversation"
                    )
                ...
                for fact_text in extracted_facts:
                    try:
                        await self.core_memory.add_learned_knowledge(
                            user_id, fact_text, "normal", "conversation"
                        )
                        result["facts_extracted"] += 1
        ...
        if result["facts_extracted"] > 0 or result["indexed"]:
            logger.info(
                f"Processed exchange: {result['facts_extracted']} facts, indexed={result['indexed']}"
            )
    ...
    async def _generate_summary_with_llm(self, conversation_text: str) -> str | None:
...
Summary:"""
    ...
    async def search_all_memory(
        ...
        if self.vector_store:
            try:
                results["vector_memories"] = await self.vector_store.search(
                    query, user_id, limit=limit
                )
                results["past_conversations"] = await self.vector_store.search_conversations(
                    query, user_id, limit=limit
                )
            except Exception as e:
        ...
        if self.core_memory:
            try:
                results["core_facts"] = await self.core_memory.get_relevant_knowledge(
    ...
    def get_stats(self, user_id: str) -> dict[str, Any]:
        ...
        if self.core_memory:
            stats["core_memory"] = {"extraction_stats": self.core_memory.get_extraction_stats()}
    ...
    async def rebuild_vector_index(self, user_id: str) -> dict[str, int]:
        ...
        results = {"core_facts_indexed": 0, "learned_knowledge_indexed": 0, "errors": 0}
