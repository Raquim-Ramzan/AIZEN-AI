...
class CoreMemory:
    ...
    def _create_default_memory(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "core_facts": [
                {**fact, "id": f"system_{i}", "timestamp": datetime.now(timezone.utc).isoformat()} 
    ...
    async def get_system_prompt_context(self, user_id: str) -> str:
        ...
        # Add identity facts
        identity_facts = [f for f in memory.get("core_facts", []) if f.get("category") == "identity"]
        if identity_facts:
        ...
        other_facts = [f for f in memory.get("core_facts", []) 
    ...
    async def add_core_fact(
        ...
        fact_entry = {
            "id": f"fact_{datetime.now(timezone.utc).timestamp()}",
            "fact": fact,
            "category": category,
    ...
    async def clear_all_facts(self, user_id: str, keep_identity: bool = True):
        ...
        if keep_identity:
            memory["core_facts"] = [
                f for f in memory.get("core_facts", [])
                if f.get("source") == "system"
            ]
        else:
            memory["core_facts"] = []
    ...
    async def add_learned_knowledge(
        ...
        knowledge_entry = {
            "id": f"knowledge_{datetime.now(timezone.utc).timestamp()}",
            "fact": fact,
            "timestamp": datetime.now(timezone.utc).isoformat(),
        ...
        if len(memory["learned_knowledge"]) > 100:
            ...
            memory["learned_knowledge"].sort(
                key=lambda x: (importance_order.get(x.get("importance", "normal"), 2), x.get("timestamp", ""))
            )
    ...
    async def extract_facts_only(self, user_id: str, user_message: str, assistant_response: str) -> List[str]:
        """Extract facts from conversation without storing them."""
        memory = await self._get_or_create_profile(user_id)
        if "extraction_stats" not in memory:
            memory["extraction_stats"] = {"total_extractions": 0, "successful_llm_extractions": 0, "fallback_extractions": 0}
    ...
    async def extract_and_store_facts(self, user_id: str, user_message: str, assistant_response: str) -> List[Dict[str, Any]]:
...
JSON output:"""
    ...
    async def _extract_facts_with_regex(self, user_message: str) -> List[Dict[str, Any]]:
        ...
        patterns = [
            (r"(?:i am|i'm|my name is|call me)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)", "user_info", "high"),
            ...
            (r"(?:remember that|don't forget|keep in mind that)\s+(.+?)(?:\.|$)", "explicit_memory", "high"),
        ]
            ...
            for match in matches:
                if len(match) > 3:
                    ...
                    if category == "user_info":
                        ...
                        if "prefer" in message_lower or "like" in message_lower or "love" in message_lower:
    ...
    async def get_relevant_knowledge(self, user_id: str, query: str, limit: int = 5) -> List[Dict[str, Any]]:
