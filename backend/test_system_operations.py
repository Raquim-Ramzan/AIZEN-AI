...
async def test_process_management():
    ...
    try:
        ...
        if result["success"]:
            print("   ✓ Listed top 5 processes by CPU usage:")
...
async def test_system_info():
    ...
    try:
        ...
        if result["success"]:
            ...
            print(
                f"     Available: {mem['virtual']['available_gb']} GB ({100 - mem['virtual']['percent']:.1f}%)"
            )
        else:
            print(f"   ✗ Failed: {result.get('error')}")
...
async def test_registry():
    ...
    try:
        ...
        result = await reg_ops.list_subkeys(path="HKEY_CURRENT_USER\\Software")
        if result["success"]:
