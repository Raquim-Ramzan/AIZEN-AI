                ...
                try:
                    ...
                    with open(file_path, "rb") as f:
                        client.storage.from_(audio_bucket).upload(
                            path=str(rel_path), file=f, file_options={"upsert": "true"}
                        )
...
async def main():
    ...
    if not client:
        logger.error(
            "Failed to initialize Supabase client. Check SUPABASE_URL and SUPABASE_KEY in .env"
        )
        return
